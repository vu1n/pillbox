...
# Resolve the runner image the SAME way pillbox does (the pillbox.toml cascade), so
# the preflight + preview match what `pillbox run` will actually materialize — instead
# of forcing our own tag and overriding the descriptor. Precedence mirrors the binary:
# $PILLBOX_RUNNER_IMAGE > project pillbox.toml (walk-up) [runner] image >
# ~/.pillbox/global/pillbox.toml [runner] image > built-in default. The `image =` key
# only appears under [runner] in the schema, so a plain extract is unambiguous.
toml_image() { sed -n 's/^image *= *"\([^"]*\)".*/\1/p' "$1" 2>/dev/null | head -1; }
cascade_image() {
  local d="$PWD" img
  while [ "$d" != "/" ]; do
    if [ -f "$d/pillbox.toml" ]; then
      img="$(toml_image "$d/pillbox.toml")"; [ -n "$img" ] && { printf '%s' "$img"; return; }
      break  # first descriptor = the selected project; fall through to global if it set no image
    fi
    d="$(dirname "$d")"
  done
  img="$(toml_image "$HOME/.pillbox/global/pillbox.toml")"; [ -n "$img" ] && { printf '%s' "$img"; return; }
  printf 'ghcr.io/vu1n/pillbox-runner:latest'  # the binary's built-in default
}
# IMAGE_OVERRIDE: only force PILLBOX_RUNNER_IMAGE on the run when it's an explicit
# override (env var or a profile pin); the default path stays flagless so the cascade
# governs (and we never drift from the binary's own resolution at run time).
if [ -n "${PILLBOX_RUNNER_IMAGE:-}" ]; then
  IMAGE="$PILLBOX_RUNNER_IMAGE"; IMAGE_OVERRIDE=1
else
  IMAGE="$(cascade_image)"; IMAGE_OVERRIDE=0
fi
...
load_profile() { # sets the GLOBAL choice vars (AGENT/BACKEND/...) from a profile file
  ...
  [ -n "$img" ] && { IMAGE="$img"; IMAGE_OVERRIDE=1; }
...
# ── compose + run ────────────────────────────────────────────────────────
envs=()
# Leverage the cascade: libkrun is the binary's default backend, so only flag docker;
# the runner image comes from the descriptor cascade, so only pass PILLBOX_RUNNER_IMAGE
# on an explicit override (env/profile) — the default run stays flagless, the cascade
# governs, and we never drift from the binary's own run-time resolution.
[ "$BACKEND" = docker ] && envs+=("PILLBOX_BACKEND=docker")
[ "$IMAGE_OVERRIDE" -eq 1 ] && envs+=("PILLBOX_RUNNER_IMAGE=$IMAGE")
...
echo; printf '%s\n' "$(bold "Command")"
# envs[] may be empty now (cascade default = no forced env). Guard: bash 3.2 under
# `set -u` errors on an empty-array expansion, so only print the env line when non-empty.
[ "${#envs[@]}" -gt 0 ] && printf '  %s \\\n' "${envs[@]}"
...
# `${envs[@]+...}` so an empty envs[] expands to nothing (bash 3.2 / set -u safe);
# `env` with no assignments just runs the command in the inherited environment.
