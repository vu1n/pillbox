...
enforces one sealed execution request, addresses an isolated sandbox on an explicitly selected, supported managed
...
- Workspace transfer uses a fresh, prefix-scoped R2 credential per transfer.
  The bucket-wide parent key never crosses to any execution provider, and the scoped
...
> Ratified by the maintainer on 2026-09-23: DigitalOcean is an optional managed
> substrate; Cloudflare remains the default. Production enablement and native
> session handoff require their documented validation gates.
