...
|---|---|---|---|
| 1 | **Remotes collapse — `docker://` + cold-host DX contract** *(foreground run + ingest + result-extraction + sandbox-side vault ✓ live-verified; **`--detach` lifecycle ✓ live-verified** — record/list/reattach/teardown-over-endpoint, agent-stays-interactive-on-reattach is a follow-on; remaining: version-skew, pull-progress)* | Cheapest big win; a *deletion* onto Docker contexts; fixes the documented product failure. **Independent of §0** — detach keys off the durable `Session.id`. | remotes + dx |
