...
| `pillbox session transcript FILE --session-id ID` | Drain a Claude Code transcript (`~/.claude/projects/<encoded>/<uuid>.jsonl`) and emit one OTLP child span per rendered event (user prompt / assistant text / tool use / tool result), parented under the session span derived from `ID`. Requires `OTEL_EXPORTER_OTLP_ENDPOINT` to actually ship; runs the parser regardless and reports the event count. See [docs/observability.md](./docs/observability.md). |
