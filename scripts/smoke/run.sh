...
if [ "$which" = all ] || [ "$which" = libkrun ]; then
  ...
  scripts/smoke/libkrun.sh opencode "${OPENCODE_IMAGE:-pillbox-runner:dev}" \
  ...
  scripts/smoke/dispatch.sh "${OPENCODE_IMAGE:-pillbox-runner:dev}" \
    "${SMOKE_MODEL:-zai-coding-plan/glm-4.5-air}" || rc=1
  ...
  scripts/smoke/dispatch-segments.sh "${OPENCODE_IMAGE:-pillbox-runner:dev}" \
    "${SMOKE_MODEL:-zai-coding-plan/glm-4.5-air}" || rc=1
fi
