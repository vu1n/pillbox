...
if (typeof pillboxName !== "string" || pillboxName.length === 0) {
  fail("live finalize requires --pillbox or BURNIN_PILLBOX_NAME for rustic snapshot verification");
}
...
if (typeof requestedBaseSnapshot !== "string" || !/^[0-9a-f]{64}$/.test(requestedBaseSnapshot)) {
  fail("operator finalize request workspace.snapshot must be a canonical base snapshot handle");
}
const expectedRequestDigest = digestText(canonicalJson(finalizeRequest));
const expectedFinalizeId = digestText(canonicalJson({
...
function dryRunPlan(value) {
  return {
    ...
    sequence: ["Pillbox --preflight records the real local managed-Codex rejection without HTTP execution or finalize", "Huddles validates the unconsumed allowance, records the only created execute, exact retry, and two bounded private status reads, then validates the consumed allowance", "Pillbox --finalize validates the Huddles report-v3 partial and rustic ancestry, then attaches scoped public cleanup to that same file"],
    safety: {
...
async function verifySnapshotAncestry({ resultSnapshot, requestedBaseSnapshot, capturePath }) {
  ...
  let stdout;
  try {
    ({ stdout } = await execFileAsync("pillbox", [
      "snapshot",
      "show",
      resultSnapshot,
      "--json",
...
function validateManifest(value) {
  ...
  if (value.workload.expected_network_requests !== networkSteps.length + 2) fail("manifest network request count must include both live allowance reads");
