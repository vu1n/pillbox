...
`,
  ...
  assert.deepEqual(Object.keys(attachment.capture).sort(), [
    "preflight",
    "source",
  ]);
  assert.equal(
    attachment.capture.preflight.error_code,
  ...
  const finalizeRequest = {
    sessionId: manifest.workload.steps.find((step) => step.id === "finalize")
      .session_id,
    workspace: {
      repo: {
...
`,
  ...
  assert.match(
    mismatchedDigest.stderr,
    /canonical request digest, finalize ID/,
  );
  assert.equal(
    JSON.parse(await readFile(reportPath, "utf8")).capture.cleanup,
    null,
  );
  ...
  assert.match(
    mismatchedParent.stderr,
    /not descended from the requested base snapshot/,
  );
  assert.equal(
    JSON.parse(await readFile(reportPath, "utf8")).capture.cleanup,
    null,
  ...
  assert.equal(
    completed.capture.cleanup.session_id,
    completed.capture.execution_identity.session_id,
  );
  ...
  assert.equal(
    completed.capture.cleanup.requested_base_snapshot,
    finalizeRequest.workspace.snapshot,
  );
  assert.equal(completed.capture.cleanup.result_snapshot, resultSnapshot);
  assert.deepEqual(requests, [
    "/v2/workspaces/finalize",
    "/v2/workspaces/finalize/status",
  ]);
  assert.equal(requestBodies[0], requestBodies[1]);
  assert.equal(finalizeEffects, 1);
  assert.deepEqual(
    completed.capture.runtime_calls,
    report.capture.runtime_calls,
  );
  assert.deepEqual(
    completed.capture.run_cost_envelopes[0].execution_identity,
    completed.capture.execution_identity,
  );
  ...
  assert.equal(
    requests.length,
    2,
    "cleanup may attach only once and never predate execution",
  );

  completed.capture.run_cost_envelopes[0].observed =
    reviewedManifest.capture.run_cost_envelopes[0].observed;
  ...
  const reconciliation = await run(process.execPath, [
    reconciler.pathname,
    reportPath,
  ]);
  ...
  assert.match(
    ...
    /managed preview burn-in reconciliation passed/,
  );
});
