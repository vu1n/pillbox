...
use opendal::{
    Metadata,
    blocking::Operator,
    layers::{ConcurrentLimitLayer, HttpClientLayer, LoggingLayer, RetryLayer, ThrottleLayer},
...
impl OpenDALBackend {
    ...
    pub fn new(path: impl AsRef<str>, options: BTreeMap<String, String>) -> RusticResult<Self> {
        Self::new_inner(path, options, None)
    }

    /// Create a new OpenDAL backend using a caller-supplied HTTP client.
    ///
    /// This constructor is intentionally additive: callers that do not need
    /// transport instrumentation continue through [`Self::new`] and retain
    /// the existing OpenDAL client. The supplied client is installed before
    /// the retry layer so every OpenDAL attempt reaches it independently.
    pub fn new_with_http_client(
        path: impl AsRef<str>,
        options: BTreeMap<String, String>,
        http_client: HttpClient,
    ) -> RusticResult<Self> {
        Self::new_inner(path, options, Some(http_client))
    }
    ...
    fn new_inner(
        ...
        if let Some(http_client) = http_client {
            operator = operator.layer(HttpClientLayer::new(http_client));
        }

        let mut operator =
            operator.layer(RetryLayer::new().with_max_times(max_retries).with_jitter());
