...
export interface WorkspaceFinalizeStore {
  ...
  get(sessionId: string): Promise<WorkspaceFinalizeRecord | null>;
  complete(input: {
    readonly finalize_id: `sha256:${string}`;
...
export class D1WorkspaceFinalizeStore implements WorkspaceFinalizeStore {
  ...
  async claim(input: {
    ...
    const inserted = await this.database
      ...
      .run();
    const record = await this.get(input.session_id);
    if (record === null)
  ...
  async get(sessionId: string): Promise<WorkspaceFinalizeRecord | null> {
    const result = await this.database
      .prepare(
       ...
       FROM workspace_finalize WHERE session_id = ? LIMIT 1`,
      )
      .bind(sessionId)
      .all<WorkspaceFinalizeRecord>();
  ...
  async complete(input: {
    ...
    const result = await this.database
      .prepare(
       ...
       WHERE finalize_id = ? AND status = 'running'`,
      )
      .bind(input.result_snapshot, input.now_ms, input.finalize_id)
      .run();
