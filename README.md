...
- An agent with shell access exfiltrating the credentials it was *given*. A
  ...
  evil.com` leaks its own token. The v0.2 vault tier addresses this for
...
- Compromised Docker daemon, container escape, or kernel-level attacks.
  Pillbox uses standard Docker isolation. v0.3 ships `pillbox run --strict`
  for hardware-isolated microVMs via Gondolin.
...
Pre-alpha. v0.2 supports Claude Code + Codex on macOS. Linux and Windows
work *in theory* (Docker + `dirs::home_dir()` exist) but haven't been tested.
...
- **v0.3** — vault tier (stub creds + egress proxy swap for API keys + GitHub
  PATs); `pillbox run --strict` (Gondolin microVMs)
- **v0.4** — Per-project config (`pillbox.toml`), per-run `--with <secret>`
  scoping

## Build (v0.2 requires the lum-built runner image)
