...
class Pillbox:
    ...
    def rm(self, sid: str):
        # `session rm` now reaps the session's microVM at the source (by spec path,
        # robust to the reparent/own-group drift that the old loop-side `os.kill`
        # worked around). Discard the "✓ removed" line but let stderr through, so a
        # reap warning surfaces in the campaign log instead of being swallowed.
        subprocess.run([self.cfg.pillbox, "session", "rm", sid],
                       stdout=subprocess.DEVNULL, env=self.env, timeout=60)
