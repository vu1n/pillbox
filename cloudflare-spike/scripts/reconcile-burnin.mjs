...
allowedKeys(capture, ["source", "notes", "execution_identity", "runtime_calls", "pre_execution_allowance_snapshot", "allowance_snapshot", "preflight", "cleanup", "run_cost_envelopes", "read_only", "totals", "operator_capture_required"], "fixture.capture");
for (const key of ["source", "execution_identity", "runtime_calls", "pre_execution_allowance_snapshot", "allowance_snapshot", "preflight", "cleanup", "run_cost_envelopes", "read_only", "totals"]) {
...
check(expectedNetworkRequests === runtimeSteps.length + cleanupSteps.length + 2, "workload network request count must include both live allowance reads");
...
validateAllowanceSnapshot(capture.pre_execution_allowance_snapshot, "capture.pre_execution_allowance_snapshot", 0);
validateAllowanceSnapshot(capture.allowance_snapshot, "capture.allowance_snapshot", reviewedCount);
...
if (partial) {
  ...
  exactKeys(cleanup, ["step_id", "operation", "http_status", "session_id", "request_count", "disposition", "finalize_id", "request_digest", "requested_base_snapshot", "result_snapshot"], "capture.cleanup");
  ...
  check(typeof cleanup.requested_base_snapshot === "string" && /^[0-9a-f]{64}$/.test(cleanup.requested_base_snapshot), "capture.cleanup.requested_base_snapshot must be a canonical snapshot handle");
...
for (const [index, rawRun] of runs.entries()) {
  ...
  check(analyticsPointsPlanned === 1, `${runId} must plan exactly one Analytics Engine point`);
...
function validateAllowanceSnapshot(value, path, expectedReserved) {
  ...
  exactKeys(snapshot, ["source", "captured_at", "deployment_epoch", "execution_limit", "reserved_executions"], path);
  check(snapshot.source === "pillbox-d1-live", `${path} must come from the live Pillbox D1 row`);
  check(
  ...
  check(snapshot.deployment_epoch === allowanceEpoch, `${path} epoch does not match the reviewed deployment epoch`);
  check(integer(snapshot.execution_limit, `${path}.execution_limit`) === allowanceLimit, `${path} limit does not match the reviewed deployment limit`);
  check(integer(snapshot.reserved_executions, `${path}.reserved_executions`) === expectedReserved, `${path} does not prove reserved_executions ${expectedReserved}`);
}
...
function validateCost(value, path) {
  ...
  check(infra.analytics_points_planned === 1, `${path}.infrastructure.analytics_points_planned must be exactly one point`);
